...
## Skills
- Current skills: `battletest`, `checkpoint`, `cleanup`, `cluster-blame`, `cluster-check`, `cluster-monitor`, `cluster-optimise`, `competition-submit-check`, `consider`, `create-plan`, `danger-check`, `decisions`, `execute`, `explain-trader`, `familiarize`, `gh-address-comments`, `gh-fix-ci`, `git-commit`, `git-merge`, `git-review`, `git-summary`, `git-sync`, `investigate`, `jupyter-notebook`, `learnings`, `notion-report`, `openai-docs`, `organise-docs`, `plan`, `playwright`, `prime`, `review-branch`, `setup`, `summarize`, `tech-debt`, `verify`, `wait-for-job`, `yeet`.
...
- Skill definition changes are also enforced in CI via `.github/workflows/skills-validation.yml`.
