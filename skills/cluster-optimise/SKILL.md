...
- For large or long optimization efforts, run recurring checkpoint cycles instead of waiting for a single end-of-task wrap-up.
...
- At the same milestone, invoke `organise-docs` when new durable learnings or decisions exist, and prune stale `plan/` scratch artifacts.
