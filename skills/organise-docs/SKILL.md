...
- In shared repositories, assume concurrent edits from humans/agents are normal and avoid reverting unknown changes while organizing docs.
- When commits are in scope, stage only files intentionally changed for the current task and exclude unrelated concurrent edits.
