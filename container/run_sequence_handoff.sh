...
$context"
  ...
  retry_codex_operation "Codex stage execution for $stage_name" "docker exec '$CONTAINER_NAME' bash -lc 'cd /workspace && cat /tmp/stage_prompt.txt | codex exec --dangerously-bypass-approvals-and-sandbox --skip-git-repo-check --config approval_policy=\"never\" --config model=\"gpt-5.1-codex-max\" --config model_reasoning_effort=\"high\"'"
