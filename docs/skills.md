...
- For long-running Slurm monitoring where Codex should wait patiently, poll intermittently, and intervene only on systemic failure/low-learning conditions (for example repeated OOM rates crossing policy thresholds), use `skills/cluster-monitor/SKILL.md` (patient monitoring loop, conservative intervention thresholds, decisive cleanup/fix/resubmit when warranted, and immediate post-completion sync/analysis).
